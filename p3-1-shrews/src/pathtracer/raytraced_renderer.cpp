...
//}
    ...
    void RaytracedRenderer::raytrace_tile(int tile_x, int tile_y, int tile_w, int tile_h) {
        // Existing code for iterating over the pixels in the tile
        size_t tile_start_x = tile_x;
        size_t tile_start_y = tile_y;
        size_t tile_end_x = std::min(tile_start_x + tile_w, frame_w);
        size_t tile_end_y = std::min(tile_start_y + tile_h, frame_h);

        // Existing code for raytracing the pixels in the tile
        for (size_t y = tile_start_y; y < tile_end_y; y++) {
            for (size_t x = tile_start_x; x < tile_end_x; x++) {
                pt->raytrace_pixel(x, y);
            }
        ...
        // Existing code for updating the frame buffer and tile samples
        pt->write_to_framebuffer(frameBuffer, tile_start_x, tile_start_y, tile_end_x, tile_end_y);
