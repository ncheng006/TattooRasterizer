...
Ray Camera::generate_ray(double x, double y) const {
  ...
  //
    Vector3D bottomleft = Vector3D(-tan(0.5 * radians(hFov)), -tan(0.5 * radians(vFov)), -1);
    Vector3D topright = Vector3D(tan(0.5 * radians(hFov)), tan(0.5 * radians(vFov)), -1);

    Vector3D pt = Vector3D(x * topright.x + (1 - x) * bottomleft.x, y * topright.y + (1 - y) * bottomleft.y, -1);
    Vector3D ray = c2w*pt;
    ray.normalize();
    Ray res = Ray(pos, ray);
    res.min_t = nClip;
